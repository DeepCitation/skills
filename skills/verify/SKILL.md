...
the coordinate match anchors each `sourceMatch` to a page/line. **lavish-axi renders the report**
and drives the review loop. We do not use DeepCitation to render the final HTML.
...
If `.deepcitation/<name>.txt` already exists from an earlier `prepare`, skip step 2.
...
2. **Prepare evidence.** `prepare` is the only reader — built-in PDF/OCR/office/web.
   ```bash
   mkdir -p .deepcitation .lavish
   npx -y deepcitation@latest prepare <file-or-url> > .deepcitation/<name>.txt 2>&1
   ```
   Multiple sources: one parallel `prepare` each (`&` + `wait`). Output carries `attachmentId`
   plus `deepTextPages` with `<page_number_N_index_I>` / `<line id="K">` tags. On **"action
   needed"** follow [rules/auth.md](rules/auth.md); on sandbox/network behavior see
...
3. **Author + coordinate-match.** Write the cited narrative and, for each fact, the anchor record
   ...
   This **is** the coordinate match. All `k`/`l`/`f` and Format 1/2 hard rules live unchanged in
   [rules/citation-anchors.md](rules/citation-anchors.md). For 100+ pages across 3+ files, split
   the work per [rules/parallel-generation.md](rules/parallel-generation.md) (Format 1 only for
   anchors; subagents emit per-section bodies + `<<<CITATION_DATA>>>`, the main loop assembles
   them into the artifact).

4. **Cheap audit → badges.** Before the first render, run the cheap auditor (haiku/sonnet) once
   per citation to assign `verified | review | contradicted` and a confidence — see the tiered
   trigger and rubric in [rules/runtime-roster.md](rules/runtime-roster.md). Never show a badge a
   citation did not earn; a citation with no located `p`/`l` anchor is never `verified`.
...
7. **Poll loop.** `npx -y lavish-axi poll .lavish/<topic>-verify.html` — long-running by design;
   ...
   returned prompt: element click vs. `target.type==="text-range"` (whose `text` is the exact
   flagged bytes). Full command contract in [rules/lavish-loop.md](rules/lavish-loop.md).

8. **Escalate on flag or low confidence.** For any citation the user flagged OR any `review`/low
   badge, run the 3-vote adversarial judge panel + editor per
   [rules/runtime-roster.md](rules/runtime-roster.md). High-confidence, unflagged citations do not
   re-run.
