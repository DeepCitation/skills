...
- For fields requiring clinical judgment: draft the best supported answer when evidence exists, then state plainly in `Review required` what the physician must confirm
...
- **Keep copy blocks clean** — citations, missing-data notes, review caveats, and source labels belong outside `Copy to form`
- **Physician judgment overlay** — fields asking for clinical opinion get evidence citations plus a plain-language note in `Review required` of what the physician must confirm
